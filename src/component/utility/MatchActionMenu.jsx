...
const MatchActionMenu = ({ row, onDelete, onEdit, onView }) => {
  ...
  // Match rows don't have a "name". Compose one from the team names so the
  // confirmation text reads "Home vs Away" whether the backend sends
  // homeTeamName (from MatchResponseDto) or a nested homeTeam object
  // (from the Match entity).
  const homeName =
    row.homeTeamName || row.homeTeam?.name || "the home team";
  const awayName =
    row.awayTeamName || row.awayTeam?.name || "the away team";
          ...
          <div className={dashboard["card_body"]}>
            ...
            >
              <p className={dashboard["alert-message"]}>
                Are you sure you want to delete {matchLabel}?
              </p>
            </Typography>
          </div>
        </div>
