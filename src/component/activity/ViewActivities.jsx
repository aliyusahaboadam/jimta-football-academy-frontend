...
const ViewActivities = () => {
  ...
  // Determine mode from the URL segment
  ...
  useEffect(() => {
    if (mode === "videos") dispatch(getVideoActivities());
    else if (mode === "images") dispatch(getImageOnlyActivities());
    else dispatch(getAllActivities());
  }, [dispatch, mode, location.pathname]);
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteActivityById(id)).unwrap();
  ...
  const handleEdit = (id) => navigate(`/admin/activities/update/${id}`);
  const handleViewDetails = (id) => navigate(`/admin/activities/details/${id}`);

  const heading =
    mode === "videos" ? "Video Activities"
      : mode === "images" ? "Image Activities"
      : "All Activities";
              ...
              <div className={dashboard["secondary--container"]}>
                <div className={[dashboard["card--add"], dashboard["card--primary"]].join(" ")}>
                  <div className={dashboard["card_body"]}>
                    ...
                    {mode === "all" && (
                      <button onClick={() => navigate("/admin/activities/add")}
                ...
                {list.length === 0 ? (
                  <div className={dashboard["empty"]}>
                    ...
                    <p className={dashboard["empty__text"]}>
                      {mode === "videos" ? "No activities with videos yet."
                        : mode === "images" ? "No image-only activities yet."
                        : "Click \"Add Activity\" to create the first one."}
                    </p>
                  </div>
                ) : (
