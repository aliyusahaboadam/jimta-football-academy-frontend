...
const UpdateActivity = () => {
  ...
  // Existing photos (S3 keys) + new photos (files) tracked separately
  const [existingPhotos, setExistingPhotos] = useState([]);   // ["activities/a.jpg", ...]
  const [newPhotos, setNewPhotos] = useState([]);             // [{ file, previewUrl, name }]

  const activityState = useSelector((state) => state.activities);
  const { updateStatus, uploadStatus } = activityState;
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        setExistingPhotos(Array.isArray(result.photoUrls) ? result.photoUrls : []);
      } catch (error) {
        setAlertType("error");
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const result = await dispatch(
        updateActivity({ id: Number(id), activityData: payload })
      ).unwrap();

      setAlertType("success");
      setMessage(result.message || "Activity updated successfully");
            ...
            >
              {({ errors, handleChange, handleSubmit, values, isSubmitting, touched, handleBlur }) => (
                <div className={style.form}>
                  ...
                  <p className={style["form-header"]}>Update Activity</p>
                  ...
                  <TextField label="Date" type="date" fullWidth margin="normal"
                    onChange={handleChange} onBlur={handleBlur}
                    value={values.activityDate} name="activityDate"
                    error={touched.activityDate && Boolean(errors.activityDate)} helperText={touched.activityDate && errors.activityDate}
                  ...
                  <TextField label="Location" fullWidth margin="normal"
                    ...
                    error={touched.location && Boolean(errors.location)} helperText={touched.location && errors.location}
                    slotProps={{ formHelperText: { sx: { fontSize: 15 } }, input: { style: { fontSize: 18 } }, inputLabel: { style: { fontSize: 16 } } }}
                  />
                  ...
                  {existingPhotos.length > 0 && (
                    <div className={dashboard["photo__grid"]}>
                      {existingPhotos.map((key) => (
                        <div key={key} className={dashboard["photo__tile"]}>
                          <img src={PHOTO_BASE + key} alt="Existing" />
                          <IconButton
                  ...
                  <Button variant="outlined" component="label"
                    ...
                    <input type="file" hidden accept="image/*"
                      onChange={(e) => { const f = e.currentTarget.files[0]; addNewPhoto(f); e.target.value = ""; }} />
                    Add another photo (max 1 MB each)
                  </Button>

                  <h3 className={style["form-section"]}>Videos (optional)</h3>

                  <TextField label="Facebook Video URL" fullWidth margin="normal"
                    onChange={handleChange} onBlur={handleBlur}
                    value={values.facebookUrl} name="facebookUrl"
                    error={touched.facebookUrl && Boolean(errors.facebookUrl)} helperText={touched.facebookUrl && errors.facebookUrl}
                    slotProps={{ formHelperText: { sx: { fontSize: 15 } }, input: { style: { fontSize: 18 } }, inputLabel: { style: { fontSize: 16 } } }}
                  />

                  <TextField label="Instagram Video URL" fullWidth margin="normal"
                    onChange={handleChange} onBlur={handleBlur}
                    value={values.instagramUrl} name="instagramUrl"
                    error={touched.instagramUrl && Boolean(errors.instagramUrl)} helperText={touched.instagramUrl && errors.instagramUrl}
                    slotProps={{ formHelperText: { sx: { fontSize: 15 } }, input: { style: { fontSize: 18 } }, inputLabel: { style: { fontSize: 16 } } }}
                  />

                  <TextField label="TikTok Video URL" fullWidth margin="normal"
                    onChange={handleChange} onBlur={handleBlur}
                    value={values.tiktokUrl} name="tiktokUrl"
                    error={touched.tiktokUrl && Boolean(errors.tiktokUrl)} helperText={touched.tiktokUrl && errors.tiktokUrl}
                    slotProps={{ formHelperText: { sx: { fontSize: 15 } }, input: { style: { fontSize: 18 } }, inputLabel: { style: { fontSize: 16 } } }}
                  />

                  <button
                    disabled={isSubmitting || updateStatus === "loading" || uploadStatus === "loading"}
                    type="submit" onClick={handleSubmit}
                    className={[style["btn"], style["btn--block"], style["btn--primary"]].join(" ")}
                  >
                    {isSubmitting || updateStatus === "loading" || uploadStatus === "loading"
                      ? "Saving..." : "Update Activity"}
                  </button>

                  <div className={style["form-link--container"]}>
                    <span className={style["form-link"]}>
                      <a className={style["link__register"]} href="/admin/activities">← Back to Activities</a>
                    </span>
                  </div>
                </div>
              )}
