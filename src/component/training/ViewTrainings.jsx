...
import { deleteTrainingById, getAllTrainings } from "../../redux/reducer/trainingSlice";
...
const ViewTrainings = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteTrainingById(id)).unwrap();
    } catch (err) {
      console.error("Delete failed:", err);
    }
  };

  const handleEdit = (id) => navigate(`/admin/trainings/update/${id}`);
  const handleViewDetails = (id) => navigate(`/admin/trainings/details/${id}`);
              ...
              <div className={dashboard["secondary--container"]}>
                <div className={[dashboard["card--add"], dashboard["card--primary"]].join(" ")}>
                  <div className={dashboard["card_body"]}>
                    <div className={dashboard["card--small-head"]}>All Trainings</div>
                    <button onClick={() => navigate("/admin/trainings/add")}
                ...
                <TableContainer component={Paper} sx={{ marginTop: 2 }}>
                  <Table sx={{ minWidth: 800 }} aria-label="trainings table">
                    <TableHead>
                      <TableRow>
                    ...
                    <TableBody>
                      ...
                      {rows.length === 0 && (
                        <StyledTableRow>
                          <StyledTableCell colSpan={8} align="center">No trainings yet. Click "Schedule Training" to create one.</StyledTableCell>
                        </StyledTableRow>
                      )}
