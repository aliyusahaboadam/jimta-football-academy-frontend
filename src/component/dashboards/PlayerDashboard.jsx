...
import { getSeasonTotals } from "../../redux/reducer/performanceSlice";
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        if (res?.id) {
          dispatch(getSeasonTotals(res.id));
          const teamId = res.teamId ?? res.team?.id;
          if (teamId) {
            dispatch(getMatchesByTeamId(teamId));
            dispatch(getPlayersByTeamId(teamId));
          }
        }
      } catch (err) {
        console.error("Player dashboard fetch failed:", err);
      }
    };
    fetch();
  }, [dispatch]);
            ...
            >
              <div className={dashboard["secondary--container"]}>
                ...
                <div className={[dashboard["card--add"], dashboard["card--primary"]].join(" ")}>
                  <div className={dashboard["card_body"]}>
                    <div className={dashboard["card--small-head"]}>
                      Welcome{player?.firstname ? `, ${player.firstname}` : ""}
                    </div>
                    <p style={{ fontSize: 14, margin: "0.5rem 0", color: "#9a99ac" }}>
                      {player?.teamName || player?.team?.name || "No team assigned yet"}
                    </p>
                  </div>
                </div>

                {/* Real season totals */}
                <div className={[dashboard["grid"], dashboard["grid--1x4"]].join(" ")} style={{ marginTop: "2rem" }}>
                ...
                <div className={[dashboard["grid"], dashboard["grid--1x3"]].join(" ")} style={{ marginTop: "1.5rem" }}>
                  ...
                  >
                    <div className={dashboard["card_body"]}>
                      <div className={dashboard["card--small-head"]}>My Team</div>
                  ...
                  >
                    <div className={dashboard["card_body"]}>
                      <div className={dashboard["card--small-head"]}>Upcoming Matches({upcomingCount})</div>
                      <div className={dashboard["card_footer"]}>View →</div>
                    </div>
                  </div>
                  ...
                  >
                    <div className={dashboard["card_body"]}>
                      <div className={dashboard["card--small-head"]}>My Performance</div>
                      <div className={dashboard["card_footer"]}>View →</div>
                    </div>
                  </div>
                </div>
                ...
                {/* Charts */}
                <DashboardCharts role="player" />

                {/* Teammates preview */}
                <h3 style={{ marginTop: "2rem", color: "#d71b3b" }}>Teammates</h3>
                <div className={[dashboard["grid"], dashboard["grid--1x4"]].join(" ")}>
                  {teammateList.slice(0, 4).map((p) => (
                    <div key={p.id} className={[dashboard["card--count"], dashboard["card--primary"]].join(" ")}>
                      <div className={dashboard["card_body"]}>
                        <div className={dashboard["card_button_and_icon"]}>
                          <span className={dashboard["icon-container"]}>
                            <svg className={[dashboard["icon--big"], dashboard["icon--primary"]].join(" ")}>
                          ...
                          <span className={dashboard["badge"]}>{p.jerseyNumber ?? "—"}</span>
                        </div>
                        {playerName(p)}
                      </div>
                    </div>
