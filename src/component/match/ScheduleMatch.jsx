...
import { saveMatch } from "../../redux/reducer/matchSlice";
...
const ScheduleMatch = () => {
  ...
  const matchSchema = object({
    ...
    homeTeamId: string().required("Home team is required"),
    awayTeamId: string()
      .required("Away team is required")
      .test("different-teams", "Home and away teams must differ", function (value) {
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      const result = await dispatch(saveMatch(payload)).unwrap();
      setAlertType("success");
      setMessage(result.message || "Match scheduled successfully");
    } catch (error) {
      setAlertType("error");
            ...
            >
              {({
                ...
                <div className={style.form}>
                  ...
                  <p className={style["form-header"]}>Schedule Match</p>
                  ...
                  <h3 className={style["form-section"]}>Teams</h3>
                  ...
                  >
                    <InputLabel sx={{ fontSize: 16 }}>Home Team</InputLabel>
                    <Select
                      label="Home Team"
                      name="homeTeamId"
                      onChange={handleChange}
                    ...
                    >
                      {teamList.length === 0 ? (
                        ...
                        teamList.map((team) => (
                          <MenuItem key={team.id} sx={{ fontSize: 18 }} value={team.id}>
                            {team.name}
                    ...
                    <FormHelperText sx={{ fontSize: 15 }}>
                      {touched.homeTeamId && errors.homeTeamId}
                    </FormHelperText>
                  </FormControl>
                  ...
                  >
                    <InputLabel sx={{ fontSize: 16 }}>Away Team</InputLabel>
                    <Select
                      label="Away Team"
                      name="awayTeamId"
                      onChange={handleChange}
                    ...
                    <FormHelperText sx={{ fontSize: 15 }}>
                      {touched.awayTeamId && errors.awayTeamId}
                    </FormHelperText>
                  </FormControl>
                  ...
                  >
                    {isSubmitting || savingStatus === "loading" ? "Saving..." : "Schedule Match"}
                  </button>
                </div>
              )}
            </Formik>
