...
import { deleteCoachById, getAllCoaches } from "../../redux/reducer/coachSlice";
import Loading from "../Chunks/loading";
import dashboard from "../style/Dashboard.module.css";
import CoachActionMenu from "../utility/CoachActionMenu";
...
const ViewCoaches = () => {
  ...
  const { coaches, fetchingStatus } = coachState;
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteCoachById(id)).unwrap();
    } catch (error) {
      console.error("Delete failed:", error);
    }
  };

  const handleEdit = (id) => navigate(`/admin/coaches/update/${id}`);
  const handleViewDetails = (id) => navigate(`/admin/coaches/details/${id}`);
                  ...
                  <Table sx={{ minWidth: 650 }} aria-label="coaches table">
                    <TableHead>
                      <TableRow>
                        ...
                        <StyledTableCell align="left">License No</StyledTableCell>
                      ...
                      {rows.length === 0 && (
                        <StyledTableRow>
                          <StyledTableCell colSpan={6} align="center">
                            No coaches yet. Click "Add New Coach" to create one.
                          </StyledTableCell>
                        </StyledTableRow>
                      )}
