...
import { getTrainingById, updateTrainingAsCoach } from "../../redux/reducer/trainingSlice";
import Loading from "../Chunks/loading";
import dashboard from "../style/Dashboard.module.css";
import style from "../style/GlobalForm.module.css";

const CoachUpdateTraining = () => {
  ...
  const [initialValues, setInitialValues] = useState({
    title: "", description: "", trainingDate: "", startTime: "", endTime: "",
    venue: "", teamId: "",
  });

  const trainingState = useSelector((state) => state.trainings);
  const { updateStatus } = trainingState;
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        if (coachRes?.id) dispatch(getTeamsByCoachId(coachRes.id));
      } catch (error) {
        setAlertType("error");
        setMessage(error?.message || "Failed to load training");
        setOpen(true);
      } finally {
        setLoading(false);
      }
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      const result = await dispatch(updateTrainingAsCoach({ id: Number(id), trainingData: payload })).unwrap();
      setAlertType("success");
      setMessage(result.message || "Training updated successfully");
            ...
            >
              {({ errors, handleChange, handleSubmit, values, isSubmitting, touched, handleBlur }) => (
                <div className={style.form}>
                  ...
                  <p className={style["form-header"]}>Update Training</p>
                  ...
                  >
                    {isSubmitting || updateStatus === "loading" ? "Saving..." : "Update Training"}
                  </button>

                  <div className={style["form-link--container"]}>
                    <span className={style["form-link"]}>
                      <a className={style["link__register"]} href="/coach/trainings">← Back to Trainings</a>
                    </span>
                  </div>
                </div>
              )}
...
export default CoachUpdateTraining;