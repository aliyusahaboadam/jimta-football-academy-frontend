...
import { sendPasswordResetPlayer } from "../../redux/reducer/passwordSlice";
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      const body = await dispatch(
        sendPasswordResetPlayer({ password: values.password })
      ).unwrap();
      setAlertType("success");
      setMessage(body.message || "Password changed successfully");
      setTimeout(() => navigate("/player/profile"), 1500);
    } catch (error) {
      setAlertType("error");
      ...
      >
        {({
          ...
          <Card>
            ...
            <p className={style["form-header"]}>Change Password</p>
            ...
            <div className={style["form-link--container"]}>
              <span className={style["form-link"]}>
                <a className={style["link__register"]} href="/player/profile">
                  ← Back to Profile
                </a>
              </span>
            </div>
