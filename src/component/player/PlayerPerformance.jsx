...
import { getPerformancesByPlayerId } from "../../redux/reducer/performanceSlice";
...
const PlayerPerformance = () => {
  ...
  const { performances, fetchingStatus } = perfState;
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        if (res?.id) dispatch(getPerformancesByPlayerId(res.id));
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const matchLabel = (p) => {
    const m = p.match;
    if (!m) return p.matchName || `Match #${p.matchId ?? "—"}`;
    const h = m.homeTeamName || m.homeTeam?.name || "Home";
    const a = m.awayTeamName || m.awayTeam?.name || "Away";
    return `${h} vs ${a}`;
  };

  const matchDate = (p) => p.match?.matchDate || p.matchDate || "—";
                ...
                <div className={[dashboard["card--add"], dashboard["card--primary"]].join(" ")}>
                  <div className={dashboard["card_body"]}>
                    ...
                    <p style={{ fontSize: 14, margin: 0, color: "#9a99ac" }}>
                      {perfList.length} {perfList.length === 1 ? "record" : "records"}
                    </p>
                  </div>
                </div>

                <TableContainer component={Paper} sx={{ marginTop: 2 }}>
                  <Table sx={{ minWidth: 800 }} aria-label="my performance table">
                    <TableHead>
                      <TableRow>
                    ...
                    <TableBody>
                      {perfList.length === 0 ? (
                        ...
                        perfList.map((row, index) => (
                          <StyledTableRow key={row.id}>
                            <StyledTableCell component="th" scope="row">
                              {index + 1}
                            </StyledTableCell>
                            <StyledTableCell align="left">{matchLabel(row)}</StyledTableCell>
                            <StyledTableCell align="left">{matchDate(row)}</StyledTableCell>
