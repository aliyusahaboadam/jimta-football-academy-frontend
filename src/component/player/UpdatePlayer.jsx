...
import { getPlayerById, updatePlayer } from "../../redux/reducer/playerSlice";
...
const UpdatePlayer = () => {
  ...
  const { updateStatus, fetchingStatus } = playerState;
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        setAlertType("error");
        setMessage(error?.message || "Failed to load player");
        setOpen(true);
      } finally {
        setLoading(false);
      }
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      const result = await dispatch(updatePlayer(payload)).unwrap();
      setAlertType("success");
      setMessage(result.message || "Player updated successfully");
      setTimeout(() => navigate("/admin/players"), 1200);
    } catch (error) {
      setAlertType("error");
            ...
            >
              {({
                ...
                <div className={style.form}>
                  ...
                  <p className={style["form-header"]}>Update Player</p>

                  <FormControl fullWidth margin="normal" error={touched.position && Boolean(errors.position)}>
                    ...
                    <Select
                      ...
                      sx={{ fontSize: 18 }}
                    >
                      <MenuItem sx={{ fontSize: 18 }} value="Goalkeeper">Goalkeeper</MenuItem>
                      <MenuItem sx={{ fontSize: 18 }} value="Defender">Defender</MenuItem>
                      <MenuItem sx={{ fontSize: 18 }} value="Midfielder">Midfielder</MenuItem>
                      <MenuItem sx={{ fontSize: 18 }} value="Forward">Forward</MenuItem>
                    </Select>
                    <FormHelperText sx={{ fontSize: 15 }}>{touched.position && errors.position}</FormHelperText>
                  </FormControl>
                  ...
                  <FormControl fullWidth margin="normal">
                    ...
                    <Select
                      ...
                      sx={{ fontSize: 18 }}
                    >
                      <MenuItem sx={{ fontSize: 18 }} value="Right">Right</MenuItem>
                      <MenuItem sx={{ fontSize: 18 }} value="Left">Left</MenuItem>
                      <MenuItem sx={{ fontSize: 18 }} value="Both">Both</MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  >
                    {isSubmitting || updateStatus === "loading" ? "Saving..." : "Update Player"}
                  </button>

                  <div className={style["form-link--container"]}>
                    <span className={style["form-link"]}>
                      <a className={style["link__register"]} href="/admin/players">
                        ← Back to Players
                      </a>
                    </span>
                  </div>
