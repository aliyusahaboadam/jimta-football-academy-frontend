...
import { getAuthenticatedAdmin } from "../../redux/reducer/adminSlice";
...
const AdminProfile = () => {
  ...
  const { admin, fetchingStatus } = adminState;
  ...
  useEffect(() => {
    dispatch(getAuthenticatedAdmin()).then((res) => {
      const id = res?.payload?.id;
      if (id) dispatch(getProfileByAdminId(id));
    });
  }, [dispatch]);
            ...
            >
              <div className={dashboard["secondary--container"]}>
                <div className={dashboard["card--details__wrapper"]}>
                  ...
                  <p className={style["form-header"]}>Admin Profile</p>

                  <div className={dashboard["card--details"]}>
                    <span>Firstname:</span> {profile?.firstname || admin?.firstname || "—"}
                  </div>
                  <div className={dashboard["card--details"]}>
                    <span>Surname:</span> {profile?.surname || admin?.surname || "—"}
                  </div>
                  <div className={dashboard["card--details"]}>
                    <span>Lastname:</span> {profile?.lastname || admin?.lastname || "—"}
                  </div>
                  <div className={dashboard["card--details"]}>
                    <span>Email:</span> {admin?.email || "—"}
                  </div>
                  <div className={dashboard["card--details"]}>
                    <span>Phone:</span> {profile?.phoneNumber || admin?.phoneNumber || "—"}
                  </div>
                  <div className={dashboard["card--details"]}>
