...
import { deleteAdminById, getAllAdmins } from "../../redux/reducer/adminSlice";
import Loading from "../Chunks/loading";
import dashboard from "../style/Dashboard.module.css";
import AdminActionMenu from "../utility/AdminActionMenu";
...
const ViewAdmins = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteAdminById(id)).unwrap();
    } catch (error) {
      console.error("Delete failed:", error);
    }
  };

  const handleEdit = (id) => navigate(`/admin/admins/update/${id}`);
  const handleViewDetails = (id) => navigate(`/admin/admins/details/${id}`);
      ...
      {fetchingStatus === "loading" ? (
        ...
        <ClickAwayListener onClickAway={handleClickAway}>
          <Box sx={{ display: "flex" }}>
            ...
            {/* Drawer — same as AddAdmin but Admins expanded */}
            <Drawer
              ...
              <div className={dashboard["secondary--container"]}>
                <div className={[dashboard["card--add"], dashboard["card--primary"]].join(" ")}>
                  <div className={dashboard["card_body"]}>
                    <div className={dashboard["card--small-head"]}>All Admins</div>
                    <button
                      ...
                      {(rowsPerPage > 0
                        ...
                        <StyledTableRow key={row.id}>
                          ...
                          <StyledTableCell component="th" align="left">
                            {[row.firstname, row.surname, row.lastname].filter(Boolean).join(" ") || "—"}
                          </StyledTableCell>
                      ...
                      {rows.length === 0 && (
                        <StyledTableRow>
                          <StyledTableCell colSpan={5} align="center">
                            No admins yet. Click "Add New Admin" to create one.
                          </StyledTableCell>
                        </StyledTableRow>
                      )}
