...
import { getAdminById } from "../../redux/reducer/adminSlice";
import { getProfileByAdminId } from "../../redux/reducer/profileSlice";
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        setData(result);
        try {
          const profile = await dispatch(getProfileByAdminId(id)).unwrap();
        ...
        >
          <div className={dashboard["secondary--container"]}>
            <div className={dashboard["card--details__wrapper"]}>
              ...
              <p className={style["form-header"]}>Admin Details</p>

              <h3 className={style["form-section"]}>Personal</h3>
              <div className={dashboard["card--details"]}>
                <span>Firstname:</span> {data?.firstname || profileData?.firstname || "—"}
              </div>
              <div className={dashboard["card--details"]}>
                <span>Surname:</span> {data?.surname || profileData?.surname || "—"}
              </div>
              <div className={dashboard["card--details"]}>
                <span>Lastname:</span> {data?.lastname || profileData?.lastname || "—"}
              </div>
              <div className={dashboard["card--details"]}>
                <span>Email:</span> {data?.email || "—"}
              </div>
              <div className={dashboard["card--details"]}>
                <span>Phone:</span> {data?.phoneNumber || profileData?.phoneNumber || "—"}
              </div>

              {profileData && (
                <>
                  <h3 className={style["form-section"]}>Profile</h3>
                  <div className={dashboard["card--details"]}>
              ...
              <div style={{ display: "flex", gap: "1rem", marginTop: "2rem" }}>
                <button
                  onClick={() => navigate(`/admin/admins/update/${id}`)}
